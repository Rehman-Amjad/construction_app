...
class PlasterController extends GetxController {
  ...
  void calculationsForFeetScreen() {
    ...
    cementBags.value = (volumeOfWall * dryVolume.value * cement.value) /
        (cement.value + sand.value);
    cementBags.value =
        (cementBags.value * 40) / oneCementBag.value * quantity.value;
    cementCost.value = cementBags.value * oneCementBagPrice.value;
    sandCalculate.value = (volumeOfWall * dryVolume.value * sand.value) /
        (cement.value + sand.value) *
        quantity.value;
  }
  ...
  void calculationsForMetersScreen() {
    ...
    plasterCost.value = plasterArea.value * plasterPrice.value;
    double volumeOfWall =
        length.value * height.value * (thickness.value / 1000);
    cementBags.value = (volumeOfWall * dryVolume.value * cement.value) /
        (cement.value + sand.value);
    cementBags.value =
        (cementBags.value * 1440) / oneCementBag.value * quantity.value;
    cementCost.value = cementBags.value * oneCementBagPrice.value;
    sandCalculate.value = (volumeOfWall * dryVolume.value * sand.value) /
        (cement.value + sand.value) *
        quantity.value;
